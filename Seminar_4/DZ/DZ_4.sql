...
SELECT cat_in_shop.name, shops.id, shops.shopname
FROM shops
JOIN (SELECT name, shops_id FROM cats
WHERE name = "Murzik") cat_in_shop
ON cat_in_shop.shops_id = shops.id; 

-- Task_3
SELECT c.name, s.id, s.shopname 
FROM cats c
JOIN shops s
ON s.id = c.shops_id
...
-- Task_4
SELECT 
	a.an_name "Название анализа",
    a.an_price "Цена анализа", 
    interval_date.ord_datetime "Дата и время заказа"
FROM analysis a
JOIN (SELECT ord_datetime, ord_an FROM orders 
WHERE ord_datetime BETWEEN '2020-02-05' AND '2020-02-12') interval_date
ON a.an_id = interval_date.ord_an
ORDER BY interval_date.ord_datetime;